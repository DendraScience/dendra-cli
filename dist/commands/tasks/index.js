  ...
  return {
    help(p) {
      return style.commandHelp({
        ...
        synopsis: [{ cmd: '<command>', opts: '[<options>] [<args>]' }, { cmd: '<command>', sub: '<sub>', opts: '[<options>] [<args>]' }, {}, { cmd: 'login', opts: '[--email=<str>] [--password=<str>]' }],
        groups: [{
          header: 'Options',
          items: [{ opts: '--email=<str>', desc: 'Email address of a registered user' }, { opts: '--password=<str>', desc: 'Login password of a registered user' }]
        }, {
          header: 'Commands',
